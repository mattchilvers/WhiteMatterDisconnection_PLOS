...
for i = 1:length(lm_data)
    ...
    Ordered_Pvalues{end+1,1} = strcat(lm_data(i).Tract,' GM Lesion Vol');
    Ordered_Pvalues{end,2} = table2array(lm_data(i).Coefs(3,4));
    Ordered_Pvalues{end+1,1} = strcat(lm_data(i).Tract,' Tract Damage');
    Ordered_Pvalues{end,2} = table2array(lm_data(i).Coefs(4,4)); 
end
...
for i = 1:length(Ordered_Pvalues)
    Ordered_Pvalues{i,3} = (i/69)*0.05; % (i/m)Q where i is the p value rank, m is the number of tests and Q is the FDR
end
...
index = [3 4 5 14 22 23];
tractorder = [5 1 6 3 2 4];
...
for i = 1:length(index)
    pos = pos+1;
    %Tract damage
        sign_data(pos,1) = lm_data(index(i)).Coefs(4,1).Estimate;
        sign_err(pos,1) = (lm_data(index(i)).Coefs(4,6).UpperCI - lm_data(index(i)).Coefs(4,5).LowerCI)/2;
        ...
        % GM_LesionVol
        sign_data(pos,3) = lm_data(index(i)).Coefs(3,1).Estimate;
        sign_err(pos,3) = (lm_data(index(i)).Coefs(3,6).UpperCI - lm_data(index(i)).Coefs(3,5).LowerCI)/2;
end
...
y = linspace(5.025,0.65,6);
y(2,:) = linspace(4.875,0.5,6);
y(3,:) = linspace(4.725,0.35,6);
y = y'; % flip data
...
% Add legend
legend('Tract Damage','Lesion Side','GM Lesion Volume','Location','best')
